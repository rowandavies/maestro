...
case class CustomerHourlyConfig(config: Config) {
  ...
  val upload    = maestro.upload (
    filePattern = "{table}?{yyyyMMddhh}*"
  )
  val load      = maestro.load[Customer](none = "null")
  val dateTable = maestro.partitionedHiveTable[Customer, (String, String, String, String)](
    partition   = Partition.byHour(Fields[Customer].EffectiveDate),
    tablename   = "by_hour"
  )
  val queries = List(
...
object CustomerHourlyJob extends MaestroJob {
  ...
  def job: Execution[JobStatus] = for {
    ...
    uploadInfo     <- uploadUTC(conf.upload)
